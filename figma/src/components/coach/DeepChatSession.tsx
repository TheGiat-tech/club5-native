...
import { deepSessions, type Message } from './deepSessions';
import { DeepChatBubble } from './DeepChatBubble';
import { DeepChoiceChips } from './DeepChoiceChips';
import { TypingIndicator } from './TypingIndicator';

export type SessionType = 'wakeup' | 'tired' | 'motivation' | 'down' | 'night';

interface DeepChatSessionProps {
...
export function DeepChatSession({ sessionType, onClose }: DeepChatSessionProps) {
  ...
  const session = sessionType ? deepSessions[sessionType] : null;
  ...
  const addMessage = (message: Message) => {
    ...
    // Simulate typing delay (longer for more human feel)
    const typingDelay = message.delay || 1500;
    setTimeout(() => {
      setIsTyping(false);
      setChatHistory(prev => [...prev, { ...message, timestamp: Date.now() }]);
      
      // If message has choices, show them after a brief pause
  ...
  const handleChoice = (choiceId: string, choiceText: string, nextId: string) => {
    ...
    // Get next message from coach with natural delay
    setTimeout(() => {
  ...
  return (
    <div className="flex-1 flex flex-col">
      {/* Header */}
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-100 px-4 py-4 flex items-center gap-3">
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto px-5 py-8 space-y-6">
        {chatHistory.map((msg, index) => (
          <DeepChatBubble key={`${msg.id}-${index}`} message={msg} />
        ))}
